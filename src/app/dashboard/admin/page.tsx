...
import { Users, Home, ClipboardList, LogOut, ShieldAlert, ShieldCheck } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: 'Banned Users', value: users.filter(u => u.isBanned).length, icon: ShieldAlert, color: 'red' },
  ];
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <a href="#users" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
            <Users size={16} /> Users
          </a>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {stats.map(stat => (
            <div key={stat.label} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-${stat.color}-100`}>
