...
import { Home, CreditCard, ClipboardList, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <a href="#rentals" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
            <ClipboardList size={16} /> My Requests
          </a>
          <a href="#payments" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
            <CreditCard size={16} /> Payments
          </a>
          <Link href="/properties" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {[
            { label: 'Total Requests', value: rentals.length, color: 'blue' },
            { label: 'Active Rentals', value: rentals.filter(r => r.status === 'ACTIVE').length, color: 'green' },
            { label: 'Completed', value: rentals.filter(r => r.status === 'COMPLETED').length, color: 'gray' },
          ].map(stat => (
